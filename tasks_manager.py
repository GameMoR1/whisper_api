...
class TasksManager:
    ...
    def get_active_tasks(self):
        with self.lock:
            return [task for task in self.tasks if task.status in ("pending", "processing")]

    def get_tasks_last_48h(self):
        cutoff = datetime.now() - self.max_history
        with self.lock:
            return [task for task in self.tasks if task.created_at >= cutoff]
